...
const Analytics = () => {
  ...
  const totalRevenue = transactions.reduce((acc: number, t: any) => acc + (t.amount_tzs || 0), 0);
  const totalSessions = sessions.length;
  const vouchersSold = vouchers.filter((v: any) => v.status === 'sold' || v.status === 'active').length;
  
  // Calculate avg session duration in minutes
  const sessionDurations = sessions
    .filter((s: any) => s.ended_at && s.started_at)
  ...
  const kpis = [
    { label: 'Total Revenue', value: totalRevenue, prefix: 'TZS ', icon: DollarSign },
    { label: 'Total Sessions', value: totalSessions, icon: Users },
    { label: 'Vouchers Sold', value: vouchersSold, icon: Ticket },
    { label: 'Avg Session', value: avgSession, suffix: ' min', icon: Clock },
  ];

  // Group vouchers by plan for the pie chart
  const planStats = vouchers.reduce((acc: any, v: any) => {
    const name = v.planName || 'Unknown';
    if (!acc[name]) acc[name] = { name, value: 0, revenue: 0, color: `hsl(${Math.random() * 360} 70% 50%)` };
    acc[name].value += 1;
    acc[name].revenue += v.price || 0;
    return acc;
  }, {});
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={planData} cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={4} dataKey="value">
                  {planData.map((entry: any, i) => <Cell key={i} fill={entry.color} />)}
                </Pie>
                <Tooltip contentStyle={{ background: 'hsl(222 50% 10%)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, fontSize: 12 }} />
              </PieChart>
            </ResponsiveContainer>
